...
class DetectFlares:
    ...
    def __calc_extremas(self):
        ...
        for i in range(1, len(self.time) - 1):
            if (self.rate[i - 1] < self.rate[i]) and (self.rate[i] > self.rate[i + 1]):
                p.append(i)
            elif (self.rate[i - 1] > self.rate[i]) and (
    ...
    def __filter_close(self):
        ...
        while i < len(self.p3):
            ...
            while j < len(self.p3):
                if (
                    self.rate[self.p3[j]] - self.rate[self.p3[i]] > MIN_COUNT_RATE_DIFF
                ) or (self.time[self.p3[j]] - self.time[self.p3[i]] > MIN_TIME_DIFF):
                    break
                j += 1
            s.append(self.s3[i])
    ...
    def __height_thresholding(self):
        ...
        heights = np.array(
            [
                (self.rate[self.p4[i]] - self.rate[self.s4[i]])
