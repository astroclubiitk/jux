...
class Star:
    ...
    def __calc_planets_influence(self):
        ...
        for i in indices[0]:
            __indices = np.where(
                np.abs(self.__corrected_fourier_power)
                != np.abs(self.__corrected_fourier_power[i])
            )
    ...
    def print_planet_details(self, n):
        ...
        if (n > len(self.planets)) or (n < 0):
            print(
                "Exoplanet not found! Enter a number between 1 and {}".format(
                    len(self.planets) + 1
                )
            )
            return
        ...
        print(
            "    Mass of the Exoplanet     = {} Earth masses".format(
                round(self.planets[n]["mass"] / ME, 2)
            )
        )
        print(
            "    Radius of Revolution      = {} AU".format(
                round(self.planets[n]["radius_revolution"] / AU, 2)
            )
        )
        print(
            "    Time Period of Revolution = {} Earth years".format(
                round(self.planets[n]["time_revolution"] / (SPD * 365), 2)
            )
        )
